...
In order to compare the performance of the different architectures of agents we must first define some way of measuring how well they performed in any 
given game. As such, we propose the following metrics:

\begin{itemize}
	\item For the townsfolk, the number of innocents that were lynched
	\item For the werewolves, the amount and duration of games won
	\item For all the other characters, how effectively they used their special ability
\end{itemize}

We expect random agents to be the least effective, because all their actions will be taken at random sometimes against their own interests. BDI agents are expected to fare better since they'll atempt cooperation through negotiation in order to take more educated guesses.
